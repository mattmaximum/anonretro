            ...
            {activeCount !== null && (
              <p className="text-text-3 text-xs">
                {isPro
                  ...
                  : <>{activeCount} of {freeLimit} active boards used · <Link to="/dashboard" className="text-accent hover:underline transition-colors">Manage boards</Link>{activeCount >= freeLimit && <> · <button onClick={() => { setUpgradeOnly(false); setShowUpgrade(true) }} className="text-accent hover:underline">Upgrade for unlimited</button></>}</>
                }
              </p>
            )}
          </>
